  ...
  <meta http-equiv="X-UA-Compatible" content="ie=edge">
    ...
    <div class="content-header">
        <div class="container-fluid">
          <div class="row mb-2">
            <div class="col-sm-6">
              <h1 class="m-0 text-dark">Halaman Chatbot Inggris</h1>
            </div><!-- /.col -->
            <div class="col-sm-6">
              <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item"><a href="?hal=dashboard">Dashboard</a></li>
                <li class="breadcrumb-item active">Chatbot Inggris </li>
              </ol>
            </div><!-- /.col -->
          </div><!-- /.row -->
        </div><!-- /.container-fluid -->
   ...
   <section class="content">
    ...
    <!-- <table class="table table-bordered data-table"> -->
              ...
              <tbody>
                                    ...
                                    <?php
                                        $no=1;
                                        $cb = mysqli_query($con,"select * from chatbot_hints_eng");
   ...
   // HAPUS
    $('body').on('click', '.hapus-post', function () {
        ...
        $.ajax({
          ...
          data:{
            id: data_id,
            tabel:'chatbot_hints_eng'
          },
          success: function(dataResult){
            var dataResult = JSON.parse(dataResult);
            if(dataResult.status=='success'){
            alert("Berhasil menghapus data."); document.location="update_IR_eng.php";
...
$(document).ready(function() {
   ...
   <?php
    if(isset($_POST['submit'])){
      ...
      if($id!=""){
        $sql = mysqli_query($con, "UPDATE chatbot_hints_eng SET question='$input', reply='$jwb', map='$map', video='$video' WHERE id_hint='$id'") or die(mysqli_error($con));
      
      if($sql){
        echo '<script>alert("Berhasil mengupdate data."); document.location="update_IR_eng.php";</script>';
      }else{
        ...
        $sql = mysqli_query($con, "INSERT INTO chatbot_hints_eng(question, reply,map,video) VALUES('$input', '$jwb','$map','$video')") or die(mysqli_error($con));
