...
// JIKA QUERY TDK DITEMUKAN
	    if (empty($array_tf_idf_query)){
	    	$output= "Sorry, your keyword is wrong";
...
// JIKA QUERY DITEMUKAN
			...
			// Select SUGGEST 1
			$sql="SELECT * FROM chatbot_hints_eng WHERE id_hint=".$id_sugesst1;
			...
			// Select SUGGEST 2
			$sql="SELECT * FROM chatbot_hints_eng WHERE id_hint=".$id_sugesst2;
			$res=mysqli_query($con, $sql);
			while($row=mysqli_fetch_assoc($res)){
			...
			// Select SUGGEST 3
			$sql="SELECT * FROM chatbot_hints_eng WHERE id_hint=".$id_sugesst3;
			$res=mysqli_query($con, $sql);
			while($row=mysqli_fetch_assoc($res)){
			...
			// Select SUGGEST 4
			$sql="SELECT * FROM chatbot_hints_eng WHERE id_hint=".$id_sugesst4;
			$res=mysqli_query($con, $sql);
			while($row=mysqli_fetch_assoc($res)){
			...
			$sql="SELECT * FROM chatbot_hints_eng WHERE id_hint=".$id_max;
			$res=mysqli_query($con, $sql);
			while($row=mysqli_fetch_assoc($res)){
			$output= $row['reply'];
			$map= $row['map'];
			...
			$output= "Sorry, your keyword is incomplete";
	...
	function get_record_akar_tf_idf($id){
		include('database.php');
		$sql="SELECT * FROM akar_tf_idf_eng WHERE id_hint=".$id;
		$res=mysqli_query($con, $sql);
	...
	function get_record_tf_idf($id){
		...
		include('database.php');
		$sql="SELECT * FROM tf_idf_eng WHERE id_hint=".$id;
		$res=mysqli_query($con, $sql);
		$array = mysqli_fetch_all ($res, MYSQLI_ASSOC);
		return $array; 
	}
